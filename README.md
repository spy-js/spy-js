...
After starting spy-js server and configuring proxy settings, open the tool UI in browser (UI URL will displayed in console output, by default it's [http://localhost:3546/](http://localhost:3546/)). Windows tray application users should see the UI opening automatically once they start the tray application.
...
When choosing between system and local proxy mode, consider the task you need to perform. If you're going to learn how some external website works by tracing its pages with potentially lots of CDN references, don't want to bother creating any session configuration files and want to just quickly use global session, then system proxy is your choice. If you're tracing your locally hosted project in your local dev environment, perhaps have specific session configuration shared across your team and don't want to bother turning on and off system wide proxy settings, then local proxy mode is your choice.
